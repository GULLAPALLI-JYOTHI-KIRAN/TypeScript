...
function OptionalParameters(name?:string):string{
    ...
    let wish=name?.toUpperCase() as string;
    return `Hello ${wish}`;
}
...
function OPtionalMoreParameter(status:boolean,port?:number,secure?:boolean):string{
...
function DefaultParameterValue(name:string="Default"):string{
